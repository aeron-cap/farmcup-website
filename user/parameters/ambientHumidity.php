...
?>
  ...
  <head>
    ...
    <title> Ambient Humidity | Farm Cup Control System </title>
  ...
  <body>
    ...
    $sqlLatest = "SELECT humidity, time_stamp FROM sensordata ORDER BY time_stamp DESC LIMIT 1;";
    $resultLatest = $conn->query($sqlLatest);
    while ($row = $resultLatest->fetch_assoc()) {
      $humidityLatest = $row["humidity"];
    ...
    $sql = "SELECT time_stamp, humidity FROM sensordata ORDER BY time_stamp";
            ...
            <div class='md-block d-md-none top-nav'>
              <div class='col-md-12 d-flex' id='button-toggle'>
                ...
                <div class='col-md-12 d-flex'>
                  <h1 class='parameter-name-title-sm'>Ambient Humidity</h1>
                </div>
              </div>
            </div>
            ...
            <div class='page-header d-md-block collapse'>
              <div class='nav-link'>
                <p class='link-top'>
                  ...
                  <a href="#">Ambient Humidity</a>
                </p>
              </div>
              <h1 class='parameter-name-title'>Ambient Humidity</h1>
            </div>
            <!---------------------------------------------------->
            <!------------------- DATE AND TIME ------------------>
            ...
            <div class='container'>
              <div class='row'>
                <div class='latest-data-container col-md-8 col-12'>
                  <!--<div class='card' style='background-color: white; padding: 1rem; border-radius: 1.5rem'>-->
                  <h2 class='parameter-name'>Updated Reading</h2>
                  <text class='parameter-last-value'><?php echo $humidityLatest; ?>%</text>
                ...
                <div class='latest-data-container col-md-4 col-12' style='background-color: #EBEDF0'>
                  This refers to the <b>moisture level in the air around the plants,</b> impacting their growth. Too high humidity can lead to diseases and mold, while low humidity may cause wilting and hinder nutrient uptake
                </div>
              </div>
            </div>
            <!---------------------------------------------------->
    ...
    <script>
      var parameterData = <?php echo json_encode(array_slice($humidityData, -50)); ?>;
    ...
    <script>
      document.getElementById('displayLast30Btn').addEventListener('click', function() {
        var parameterData = <?php echo json_encode($humidityData); ?>;
    ...
    <script>
      ...
      const originalhumidityData = <?php echo json_encode($humidityData); ?>;
      ...
      document.getElementById('apply-filter-btn').addEventListener('click', function() {
        ...
        updateChart(filteredData.timeLabels, filteredData.humidityData);
        updateTable(filteredData.dateLabels, filteredData.timeLabels, filteredData.humidityData);
      ...
      function filterData(startDate, endDate, startTime, endTime) {
        ...
        for (let i = 0; i < originalDateLabels.length; i++) {
          ...
          if (dateTime >= startDateTime && dateTime <= endDateTime) {
            ...
            filteredhumidityData.push(originalhumidityData[i]);
          }
        }
      ...
      function updateChart(timeLabels, humidityData) {
        // Your chart update code here
        console.log('Updating chart with filtered data');
        parameterChart.data.labels = timeLabels;
        parameterChart.data.datasets[0].data = humidityData;
        parameterChart.update();
      }

      function updateTable(dateLabels, timeLabels, humidityData) {
        const tableBody = document.querySelector('.parameters tbody');
