...
?>
  ...
  <head>
    ...
    <title> Ambient Light | Farm Cup Control System </title>
  ...
  <body>
    ...
    $sqlLatest = "SELECT ambient_light, time_stamp FROM sensordata ORDER BY time_stamp DESC LIMIT 1;";
    $resultLatest = $conn->query($sqlLatest);
    while ($row = $resultLatest->fetch_assoc()) {
      $lightLatest = $row["ambient_light"];
    ...
    $sql = "SELECT time_stamp, ambient_light FROM sensordata ORDER BY time_stamp";
            ...
            <div class='md-block d-md-none top-nav'>
              <div class='col-md-12 d-flex' id='button-toggle'>
                ...
                <div class='col-md-12 d-flex'>
                  <h1 class='parameter-name-title-sm'>Ambient Light</h1>
                </div>
              </div>
            </div>
            ...
            <div class='page-header d-md-block collapse'>
              <div class='nav-link'>
                <p class='link-top'>
                  ...
                  <a href="#">Ambient Light</a>
                </p>
              </div>
              <h1 class='parameter-name-title'>Ambient Light</h1>
            </div>
            <!---------------------------------------------------->
            <!------------------- DATE AND TIME ------------------>
            ...
            <div class='container'>
              <div class='row'>
                <div class='latest-data-container col-md-8 col-12'>
                  ...
                  <text class='parameter-last-value'><?php echo $lightLatest; ?> lux</text>
                ...
                <div class='latest-data-container col-md-4 col-12' style='background-color: #EBEDF0'>
                  Too little light can result in weak or stunted growth, while too much can cause sunburn or heat stress.
                </div>
              </div>
            </div>
            <!---------------------------------------------------->
    ...
    <script>
      var parameterData = <?php echo json_encode(array_slice($lightData, -50)); ?>;
    ...
    <script>
      document.getElementById('displayLast30Btn').addEventListener('click', function() {
        var parameterData = <?php echo json_encode($lightData); ?>;
    ...
    <script>
      ...
      const originallightData = <?php echo json_encode($lightData); ?>;
      ...
      document.getElementById('apply-filter-btn').addEventListener('click', function() {
        ...
        updateChart(filteredData.timeLabels, filteredData.lightData);
        updateTable(filteredData.dateLabels, filteredData.timeLabels, filteredData.lightData);
      ...
      function filterData(startDate, endDate, startTime, endTime) {
        ...
        for (let i = 0; i < originalDateLabels.length; i++) {
          ...
          if (dateTime >= startDateTime && dateTime <= endDateTime) {
            ...
            filteredlightData.push(originallightData[i]);
          }
        }
        ...
        return {
          ...
          lightData: filteredlightData
        };
      }

      function updateChart(timeLabels, lightData) {
        // Your chart update code here
        console.log('Updating chart with filtered data');
        parameterChart.data.labels = timeLabels;
        parameterChart.data.datasets[0].data = lightData;
        parameterChart.update();
      }

      function updateTable(dateLabels, timeLabels, lightData) {
        const tableBody = document.querySelector('.parameters tbody');
