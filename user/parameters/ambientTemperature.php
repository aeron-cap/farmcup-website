...
?>
  ...
  <head>
    ...
    <title> Ambient Temperature | Farm Cup Control System </title>
  ...
  <body>
    ...
    $sqlLatest = "SELECT temperature, time_stamp FROM sensordata ORDER BY time_stamp DESC LIMIT 1;";
    $resultLatest = $conn->query($sqlLatest);
    while ($row = $resultLatest->fetch_assoc()) {
      $temperatureLatest = $row["temperature"];
    ...
    $sql = "SELECT time_stamp, temperature FROM sensordata ORDER BY time_stamp";
        ...
        <div class="col-md-10">
          <div class="layout-page">
            <div class='md-block d-md-none top-nav'>
              <div class='col-md-12 d-flex' id='button-toggle'>
                ...
                <div class='col-md-12 d-flex'>
                  <h1 class='parameter-name-title-sm'>Ambient Temperature</h1>
                </div>
              </div>
            </div>
            ...
            <div class='page-header d-md-block collapse'>
              ...
              <h1 class='parameter-name-title'>Ambient Temperature</h1>
            </div>
            <!---------------------------------------------------->
            <!------------------- DATE AND TIME ------------------>
            ...
            <div class='container'>
              <div class='row'>
                <div class='latest-data-container col-md-8 col-12'>
                  ...
                  <text class='parameter-last-value'><?php echo $temperatureLatest; ?>&deg;C</text>
                ...
                <div class='latest-data-container col-md-4 col-12' style='background-color: #EBEDF0'>
                  If it's too hot, plants might wilt or suffer heat stress, while if it's too cold, their growth might slow down or even stop.
                </div>
              </div>
            </div>
            <!------
    ...
    <script>
      var parameterData = <?php echo json_encode(array_slice($temperatureData, -50)); ?>;
    ...
    <script>
      document.getElementById('displayLast30Btn').addEventListener('click', function() {
        var parameterData = <?php echo json_encode($temperatureData); ?>;
    ...
    <script>
      ...
      const originaltemperatureData = <?php echo json_encode($temperatureData); ?>;
      ...
      document.getElementById('apply-filter-btn').addEventListener('click', function() {
        ...
        updateChart(filteredData.timeLabels, filteredData.temperatureData);
        updateTable(filteredData.dateLabels, filteredData.timeLabels, filteredData.temperatureData);
      ...
      function filterData(startDate, endDate, startTime, endTime) {
        ...
        for (let i = 0; i < originalDateLabels.length; i++) {
          ...
          if (dateTime >= startDateTime && dateTime <= endDateTime) {
            ...
            filteredtemperatureData.push(originaltemperatureData[i]);
          }
        }
      ...
      function updateChart(timeLabels, temperatureData) {
        ...
        parameterChart.data.labels = timeLabels;
        parameterChart.data.datasets[0].data = temperatureData;
        parameterChart.update();
      }

      function updateTable(dateLabels, timeLabels, temperatureData) {
        const tableBody = document.querySelector('.parameters tbody');
