...
?>
  ...
  <head>
    ...
    <title> Cocopeat Moisture | Farm Cup Control System </title>
  ...
  <body>
    ...
    $sqlLatest = "SELECT moisture, time_stamp FROM sensordata ORDER BY time_stamp DESC LIMIT 1;";
    $resultLatest = $conn->query($sqlLatest);
    while ($row = $resultLatest->fetch_assoc()) {
      $moistureLatest = $row["moisture"];
    ...
    $sql = "SELECT time_stamp, moisture FROM sensordata ORDER BY time_stamp";
            ...
            <div class='md-block d-md-none top-nav'>
              <div class='col-md-12 d-flex' id='button-toggle'>
                ...
                <div class='col-md-12 d-flex'>
                  <h1 class='parameter-name-title-sm'>Cocopeat Moisture</h1>
                </div>
              </div>
            </div>
            ...
            <div class='page-header d-md-block collapse'>
              <div class='nav-link'>
                <p class='link-top'>
                  ...
                  <a href="#">Cocopeat Moisture</a>
                </p>
              </div>
              <h1 class='parameter-name-title'>Cocopeat Moisture</h1>
            </div>
            <!---------------------------------------------------->
            <!------------------- DATE AND TIME ------------------>
            ...
            <div class='container'>
              <div class='row'>
                <div class='latest-data-container col-md-8 col-12'>
                  ...
                  <text class='parameter-last-value'><?php echo $moistureLatest; ?></text>
                ...
                <div class='latest-data-container col-md-4 col-12' style='background-color: #EBEDF0'>
                  <b>Cocopeat moisture level is about how wet or dry the cocopeat</b>, a soil alternative made from coconut husks, is. Too much moisture can suffocate plant roots and promote mold, while too little can cause the plants to dry out and suffer.
                </div>
              </div>
            </div>
            <!---------------------------------------------------->
    ...
    <script>
      var parameterData = <?php echo json_encode(array_slice($moistureData, -50)); ?>;
    ...
    <script>
      document.getElementById('displayLast30Btn').addEventListener('click', function() {
        var parameterData = <?php echo json_encode($moistureData); ?>;
    ...
    <script>
      document.getElementById('displayLastHourBtn').addEventListener('click', function() {
        ...
        const last30pHData = parameterData.slice(-30);
    ...
    <script>
      ...
      const originalmoistureData = <?php echo json_encode($moistureData); ?>;
      ...
      document.getElementById('apply-filter-btn').addEventListener('click', function() {
        ...
        updateChart(filteredData.timeLabels, filteredData.moistureData);
        updateTable(filteredData.dateLabels, filteredData.timeLabels, filteredData.moistureData);
      ...
      function filterData(startDate, endDate, startTime, endTime) {
        ...
        for (let i = 0; i < originalDateLabels.length; i++) {
          ...
          if (dateTime >= startDateTime && dateTime <= endDateTime) {
            ...
            filteredMoistureData.push(originalmoistureData[i]);
          }
        }
      ...
      function updateChart(timeLabels, moistureData) {
        console.log('Updating chart with filtered data');
        parameterChart.data.labels = timeLabels;
        parameterChart.data.datasets[0].data = moistureData;
        parameterChart.update();
      }

      function updateTable(dateLabels, timeLabels, moistureData) {
        const tableBody = document.querySelector('.parameters tbody');
