...
?>
  ...
  <head>
    ...
    <title> Electrical Conductivity | Farm Cup Control System </title>
  ...
  <body>
    ...
    $sqlLatest = "SELECT EC, time_stamp FROM sensordata ORDER BY time_stamp DESC LIMIT 1;";
    ...
    $sql = "SELECT time_stamp, EC FROM sensordata ORDER BY time_stamp";
            ...
            <div class='md-block d-md-none top-nav'>
              <div class='col-md-12 d-flex' id='button-toggle'>
                ...
                <div class='col-md-12 d-flex'>
                  <h1 class='parameter-name-title-sm'>Electrical Conductivity</h1>
                </div>
              </div>
            </div>
            ...
            <div class='page-header d-md-block collapse'>
              <div class='nav-link'>
                <p class='link-top'>
                  ...
                  <a href="#">Electrical Conductivity</a>
                </p>
              </div>
              <h1 class='parameter-name-title'>Electrical Conductivity</h1>
            </div>
            <!---------------------------------------------------->
            <!------------------- DATE AND TIME ------------------>
            ...
            <div class='container'>
              <div class='row'>
                <div class='latest-data-container col-md-8 col-12'>
                  ...
                  <text class='parameter-last-value'><?php echo $ecLatest; ?>ms/cm</text>
                ...
                <div class='latest-data-container col-md-4 col-12' style='background-color: #EBEDF0'>
                  EC helps maintain the <b>right balance of nutrients for optimal plant growth.</b> Too high EC can lead to nutrient burn, while too low EC may result in nutrient deficiencies.
                </div>
              </div>
            </div>
              ...
              <div class='row'>
                <div class='col-md-6'>
                  ...
                  <div class="card-top w-100">
                    <div class="top-card">
                      <canvas id="ecChart" width="800" height="400"></canvas>
                    </div>
                  </div>
                </div>
    ...
    <script>
      var pHData = <?php echo json_encode(array_slice($ecData, -50)); ?>;
    ...
    <script>
      document.getElementById('displayLast30Btn').addEventListener('click', function() {
        var ecData = <?php echo json_encode($ecData); ?>;
        var dateLabels = <?php echo json_encode($dateLabels); ?>;
        var timeLabels = <?php echo json_encode($timeLabels); ?>;

        updateChart(timeLabels, ecData);
        updateTable(dateLabels, timeLabels, ecData);
      });

      function updateChart(timeLabels, pHData) {
        // Update the chart with the new data
        ecChart.data.labels = timeLabels;
        ecChart.data.datasets[0].data = ecData;
        ecChart.update();
      }

      function updateTable(dateLabels, timeLabels, ecData) {
    ...
    <script>
      document.getElementById('displayLastHourBtn').addEventListener('click', function() {
        ...
        const last30pHData = ecData.slice(-30);
    ...
    <!---------------- TO SHOW AND HIDE THE FILTER FORM --------------------->
    <script>
    ...
    <script>
      ...
      const originalecData = <?php echo json_encode($ecData); ?>;
      ...
      document.getElementById('apply-filter-btn').addEventListener('click', function() {
        ...
        updateChart(filteredData.timeLabels, filteredData.ecData);
        updateTable(filteredData.dateLabels, filteredData.timeLabels, filteredData.ecData);
      ...
      function filterData(startDate, endDate, startTime, endTime) {
        ...
        for (let i = 0; i < originalDateLabels.length; i++) {
          ...
          if (dateTime >= startDateTime && dateTime <= endDateTime) {
            ...
            filteredecData.push(originalecData[i]);
          }
        }
        ...
        return {
          ...
          ecData: filteredecData
        };
      }

      function updateChart(timeLabels, ecData) {
    ...
    <script>
      var timeLabels = <?php echo json_encode(array_slice($timeLabels, -50)); ?>;
      var ecData = <?php echo json_encode(array_slice($ecData, -50)); ?>;
