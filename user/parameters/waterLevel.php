...
?>
  ...
  <head>
    ...
    <title> Water Level | Farm Cup Control System </title>
  ...
  <body>
    ...
    $sqlLatest = "SELECT waterlevel, time_stamp FROM sensordata ORDER BY time_stamp DESC LIMIT 1;";
    $resultLatest = $conn->query($sqlLatest);
    while ($row = $resultLatest->fetch_assoc()) {
      $waterLevelLatest = round(((25.4 - $row["waterlevel"]) / 23.40) * 100);
    ...
    $sql = "SELECT time_stamp, waterlevel FROM sensordata ORDER BY time_stamp";
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      $dateLabels[] = $date;
      $timeLabels[] = $time;
      $waterLevelData[] = round(((25.4 - $row["waterlevel"]) / 23.40) * 100);
    }
            ...
            <div class='md-block d-md-none top-nav'>
              <div class='col-md-12 d-flex' id='button-toggle'>
                ...
                <div class='col-md-12 d-flex'>
                  <h1 class='parameter-name-title-sm'>Water Level</h1>
                </div>
              </div>
            </div>
            ...
            <div class='page-header d-md-block collapse'>
              <div class='nav-link'>
                <p class='link-top'>
                  ...
                  <a href="#">Water Level</a>
                </p>
              </div>
              <h1 class='parameter-name-title'>Water Level</h1>
            </div>
            <!---------------------------------------------------->
            <!------------------- DATE AND TIME ------------------>
            ...
            <div class='container'>
              <div class='row'>
                <div class='latest-data-container col-md-8 col-12'>
                  ...
                  <text class='parameter-last-value'><?php echo $waterLevelLatest; ?>%</text>
                ...
                <div class='latest-data-container col-md-4 col-12' style='background-color: #EBEDF0'>
                  Water level control in hydroponics ensures nutrient access, oxygenation, pH stability, disease prevention, and efficient nutrient uptake for healthy growth.
                </div>
              </div>
            </div>
            <!---------------------------------------------------->
    ...
    <script>
      var parameterData = <?php echo json_encode(array_slice($waterLevelData, -50)); ?>;
    ...
    <script>
      document.getElementById('displayLast30Btn').addEventListener('click', function() {
        var parameterData = <?php echo json_encode($waterLevelData); ?>;
    ...
    <script>
      ...
      document.getElementById('apply-filter-btn').addEventListener('click', function() {
        ...
        updateChart(filteredData.timeLabels, filteredData.waterLevelData);
        updateTable(filteredData.dateLabels, filteredData.timeLabels, filteredData.waterLevelData);
      ...
      function filterData(startDate, endDate, startTime, endTime) {
        ...
        for (let i = 0; i < originalDateLabels.length; i++) {
          ...
          if (dateTime >= startDateTime && dateTime <= endDateTime) {
            ...
            filteredwaterLevelData.push(originalwaterLevelData[i]);
          }
        }
        ...
        return {
          ...
          waterLevelData: filteredwaterLevelData
        };
      }

      function updateChart(timeLabels, waterLevelData) {
        // Your chart update code here
        console.log('Updating chart with filtered data');
        parameterChart.data.labels = timeLabels;
        parameterChart.data.datasets[0].data = waterLevelData;
        parameterChart.update();
      }

      function updateTable(dateLabels, timeLabels, waterLevelData) {
        const tableBody = document.querySelector('.parameters tbody');
